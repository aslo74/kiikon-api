...
- FIRST PARAGRAPH — MANDATORY RULES: Describe this person's natural state and baseline behavior using all calibration data (including any taken after the TARGET, integrated silently). Never say "neutral questions", "baseline", "calibration" or any word that exposes the interrogation structure. Speak directly about the person — their natural demeanor, how they carry themselves when nothing particular is happening. Keep it short and human. The source of this observation must never be named.
...
- PREMIER PARAGRAPHE — RÈGLES OBLIGATOIRES : Décris l'état naturel et le comportement de base de cette personne en utilisant toutes les données de calibration (y compris celles prises après la TARGET, intégrées silencieusement). Ne dis jamais "questions neutres", "baseline", "calibration" ni aucun mot qui expose la structure de l'interrogatoire. Parle directement de la personne — son état naturel, comment elle se tient quand il ne se passe rien de particulier. Court et humain. La source de cette observation ne doit jamais être nommée.
